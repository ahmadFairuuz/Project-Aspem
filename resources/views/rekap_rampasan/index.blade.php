...
@section('content')
    ...
    <div class="container-fluid px-4">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                {{-- Filter Form --}}
                <form method="GET" action="{{ route('rekap-barang-rampasan.index') }}"
                    ...
                    <select name="status" class="form-select-sm">
                        ...
                        <option value="Belum memiliki nilai taksir"
                            {{ request('status') == 'Belum memiliki nilai taksir' ? 'selected' : '' }}>Belum memiliki nilai
                            taksir</option>
                        <option value="Memiliki nilai taksir"
                            {{ request('status') == 'Memiliki nilai taksir' ? 'selected' : '' }}>Memiliki nilai taksir
                        </option>
                ...
                <tbody>
                    @forelse ($rekap as $item)
                        <tr>
                            ...
                            @if (in_array(Auth::user()->role, ['user', 'superadmin']))
                                <td>
                                    <a href="{{ route('rekap-barang-rampasan.edit', $item->id) }}"
                                        class="btn btn-sm btn-warning">Edit</a>
                                    <form action="{{ route('rekap-barang-rampasan.destroy', $item->id) }}" method="POST"
